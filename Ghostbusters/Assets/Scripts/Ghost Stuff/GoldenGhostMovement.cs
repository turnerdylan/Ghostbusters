...
using TMPro;
public enum GOLDEN_GHOST_STATE
{
    WANDER, //default
...
public class GoldenGhostMovement : MonoBehaviour
{
    //references
    NavMeshAgent agent;
    ...
    //private variables
    public GOLDEN_GHOST_STATE currentState = GOLDEN_GHOST_STATE.FLEE;
    private float timer;
    ...
    {
        currentState = GOLDEN_GHOST_STATE.WANDER;
        agent.speed = 5f;
        agent.acceleration = 8f;
        //wanderTimer = Random.Range(wanderTimer - 1, wanderTimer + 1);
        timer = timerUntilWanderMax;
        while(currentState == GOLDEN_GHOST_STATE.WANDER)
        {
    ...
    {
        currentState = GOLDEN_GHOST_STATE.FLEE;

        agent.speed = 7.5f;
        agent.acceleration = 200;
        
        while(currentState == GOLDEN_GHOST_STATE.FLEE)
        {
